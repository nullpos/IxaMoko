(function(){
    function main($) {
        ...
        if(localStorage.getItem(STORAGE_KEY)) {
            ...
            keepSkill.find('input:first').val(text)
            showSkillList()
        }
        ...
        startButton.on('click', function() {
            if(timerId == -1) {
                if(isValid()) {
                    let skillText = $('#keepText').val()
                    if(skillText.strip() != '') {
                        localStorage.setItem(STORAGE_KEY, $('#keepText').val())
                    }
                    startButton.val('中断')
                    init()
                    start()
        ...
        let log = (function() {
            return {
                ...
                card: function(data, removed) {
                    ...
                    let text = rare + ' No.' + data['no'] + ': ' + data['name'] + '(' + data['skill'] + ')'
        ...
        let display = (function() {
            ...
            return {
                ...
                init: function() {
                    displayDiv.append(money).append(cards).append(bids).append(card_cap)
                }
            }
        })()
